...
def _filter_collectable(resource_nodes: Iterator[WorldGraphNode], reach: GeneratorReach) -> Iterator[WorldGraphNode]:
    ...
    for resource_node in resource_nodes:
        if resource_node.can_collect(context, reach.state.energy):
            yield resource_node
...
def _filter_out_dangerous_actions(
    resource_nodes: Iterator[WorldGraphNode],
    graph: WorldGraph,
    context: NodeContext,
) -> Iterator[WorldGraphNode]:
    for resource_node in resource_nodes:
        if all(
            resource not in graph.dangerous_resources for resource, _ in resource_node.resource_gain_on_collect(context)
        ):
            yield resource_node
...
def _get_safe_resources(reach: GeneratorReach) -> Iterator[WorldGraphNode]:
    yield from _filter_reachable(
        _filter_out_dangerous_actions(
            collectable_resource_nodes(reach.safe_nodes, reach), reach.world_graph, reach.node_context()
        ),
        reach,
    )
...
def collect_all_safe_resources_in_reach(reach: GeneratorReach) -> None:
    ...
    while True:
        ...
        for action in actions:
            if action.can_collect(reach.node_context(), reach.state.energy):
