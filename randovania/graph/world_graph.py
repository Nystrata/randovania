...
def _connections_from(
    ...
    if isinstance(node.original_node, EventNode):
        if add_self_as_requirement_to_resources:
            requirement_to_leave = ResourceRequirement.simple(node.original_node.event)

    elif isinstance(node.original_node, ConfigurableNode):
        raise NotImplementedError

    elif isinstance(node.original_node, PickupNode):
        if add_self_as_requirement_to_resources:
...
def create_graph(
    ...
    for region, area, original_node in game_data.region_list.all_regions_areas_nodes:
        if original_node.is_derived_node:
            # TODO: EventPickupNodes are neat
            continue
        ...
        if isinstance(original_node, PickupNode):
            ...
            target = patches.pickup_assignment.get(original_node.pickup_index)
    ...
    return WorldGraph(
        ...
        dangerous_resources=frozenset(_dangerous_resources()),
        nodes=nodes,
